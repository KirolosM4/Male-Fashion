...
const AllProducts = () => {
    ...
    // end var 
    // start context 
    const {products,getAllProducts} = useContext(Store);
    // end context 
    // del product function
    const delProduct = (id,title,image) => {
        Swal.fire({
            title: `${title.split(" ")[0]} Will Be Deleted !`,
            icon: "warning",
            imageUrl: `${image}`,
            imageHeight: 200,
    ...
    return(
        <div className="mb-3 flex flex-col justify-evenly px-5 h-[70vh] md:grid md:grid-cols-12 md:grid-rows-12 md:h-screen md:gap-7 md:px-0 md:mb-0">
            ...
            <div className="col-start-4 col-span-7 row-start-0 row-span-3 flex flex-col gap-3 py-4 items-center text-[#475569] font-bold">
             ...
             <Card className="col-start-4 col-span-8 row-start-4 row-span-8 overflow-y-scroll bg-[#475569] text-white h-1/2 md:h-full md:max-h-fit">
                {
                    products.length == 0 
                    ?
                        ...
                        {products.map(({image,price,id,title},index) => {
                            ...
                            <tr key={index}>
                                ...
                                >
                                    ...
                                    <Button color="red" onClick={()=>delProduct(id,title,image)}>Del</Button>
                                </Typography>
                                </td>
                            </tr>
                            );
